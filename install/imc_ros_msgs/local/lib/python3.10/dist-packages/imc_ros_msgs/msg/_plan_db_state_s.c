...
bool imc_ros_msgs__msg__plan_db_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("imc_ros_msgs.msg._plan_db_state.PlanDBState", full_classname_dest, 43) == 0);
  }
  imc_ros_msgs__msg__PlanDBState * ros_message = _ros_message;
  {  // plan_count
    PyObject * field = PyObject_GetAttrString(_pymsg, "plan_count");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->plan_count = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // plan_size
    ...
    assert(PyLong_Check(field));
    ros_message->plan_size = PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // change_time
  ...
  {  // plans_info
    PyObject * field = PyObject_GetAttrString(_pymsg, "plans_info");
    if (!field) {
      return false;
    }
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'plans_info'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!imc_ros_msgs__msg__PlanDBInformation__Sequence__init(&(ros_message->plans_info), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create imc_ros_msgs__msg__PlanDBInformation__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    imc_ros_msgs__msg__PlanDBInformation * dest = ros_message->plans_info.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!imc_ros_msgs__msg__plan_db_information__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of PlanDBState */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("imc_ros_msgs.msg._plan_db_state");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "PlanDBState");
  ...
  {  // plan_count
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->plan_count);
    {
      int rc = PyObject_SetAttrString(_pymessage, "plan_count", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
  ...
  {  // plans_info
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->plans_info.data[i]);
      PyObject * pyitem = imc_ros_msgs__msg__plan_db_information__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
    ...
    {
      int rc = PyObject_SetAttrString(_pymessage, "plans_info", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
